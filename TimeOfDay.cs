...
{
    [Info("TimeOfDay", "FuJiCuRa + P.P", "2.3.5")]
    ...
    public class TimeOfDay : RustPlugin
    {
		...
		float timeToFreeze;
		float offsetNightStartTime;
		float offsetDayStartTime;
		...
		void LoadVariables()
		{
			...
			offsetDayStartTime = System.Convert.ToSingle(GetConfig("TimeOffset", "offsetDayStartTime(Native : 07:25)", 0.0));
			offsetNightStartTime = System.Convert.ToSingle(GetConfig("TimeOffset", "offsetNightStartTime(Native : 19:45)", 0.0));
        ...
        {
			if (!Initialized) return;
			if (TOD_Sky.Instance.Cycle.Hour + offsetDayStartTime > TOD_Sky.Instance.SunriseTime && TOD_Sky.Instance.Cycle.Hour < TOD_Sky.Instance.SunsetTime && !activatedDay)
			{
				OnSunrise();
				return;
			}
			if ((TOD_Sky.Instance.Cycle.Hour - (offsetNightStartTime) > TOD_Sky.Instance.SunsetTime || TOD_Sky.Instance.Cycle.Hour < TOD_Sky.Instance.SunriseTime) && activatedDay)
			{
				OnSunset();
				return;
			}
        ...
        void OnSunrise()
        {
			...
			{
				TOD_Sky.Instance.Cycle.Hour = TOD_Sky.Instance.SunsetTime - (offsetNightStartTime);
        ...
        void OnSunset()
        {
			...
			if (autoSkipNight)
			{
				...
				TOD_Sky.Instance.Cycle.Hour += timeToAdd + offsetDayStartTime;
				if (logAutoSkipConsole)
